...
class KerberosSourceSerializer(SourceSerializer):
    ...
    def get_last_sync(self, source: KerberosSource) -> KerberosSourceSyncSerializer | None:
        if last_sync := source.kerberossourcesync_set.order_by("-started_at").first():
            return KerberosSourceSyncSerializer(last_sync).data
        return None
